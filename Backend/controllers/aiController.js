...
export const recoveryPlan = async (req, res) => {
  try {
    ...
    const userMsg =`Habit: ${habit.name} (${habit.category}).\nDescription: ${habit.description || "none"}.\nCurrent streak: ${current} days. Longest streak: ${longest} days. The user just broke a streak. Write a warm, actionable 3-day recovery plan.`;
    ...
    const context = habits
      .map((h) => {
        ...
        return `${h.name} (${h.category}): ${
          hLogs.length
        }/30 in last 30 days, by weekday [Sun,Mon,Tue,Wed,Thu,Fri,Sat] = ${byDow}`;
      })
      .join("\n");
